...
(function ($) {
    ...
    function Scroller(elem, settings) {
        ...
        */
        that.setValue = function (sc, fill, time, temp) {
            ...
            school = $('.dw-school .dwwl0 .dw-sel').attr("data-val");
            ...
            if (!temp) {
                ...
                that.val = v;
                // console.log($('#info_school').html());
                // that.html += "span class="meta-school c-blue"><i class="fa fa-mars"></i></span>";
            }

            if (fill) {
                if (input) {
                    // if(school == 2015) {$('#info_school').parent().html('<p><span>性别</span></p><input id="info_school"  value ="男"  class="bounceOutUp"  ></input> <span class="meta-school c-blue"><i class="fa fa-mars"></i></span>');}
        ...
        */
        that.show = function (prevAnim) {
            ...
            for (i = 0; i < s.wheels.length; i++) {
                ...
                html += '</tr></table></div></div>'+ '</div></div></div></div>';
            }
            ...
            // Events
            dw.delegate('.dwwl', 'DOMMouseScroll mousewhe哦哦哦哦哦哦哦哦·el', function (e) {
                if (!isReadOnly(this)) {
                    e.preventDefault();
                    e = e.originalEvent;
        ...
        that.init = function (ss) {
            ...
            if (s.display == 'inline') {
                ...
                if (input && s.showOnFocus) {
                    ...
                    e.readOnly = true;
                    // Init show datewheel
                    elm.bind('focus.dw', function () { that.show(); that.fadeIn();});
                }
            }
        };
    ...
    }).bind('mouseover mouseup mousedown ', function (e) { // Prevent standard behaviour on body click
        if (tap) {
            e.stopPropagation();
            e.preventDefault();
            return false;
    ...
    $.fn.mobiscrollsc = function (method) {
        extend(this, $.mobiscrollsc.shorts);
        return init(this, method, arguments);
    };

    $.mobiscrollsc = $.mobiscrollsc || {
        /**
